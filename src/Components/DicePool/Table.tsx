...
export function Table({ pool }: Props) {
	const { qDice, roll } = useDicePool(pool);
	...
	return (
		<>
			...
			<div className={`${styles.subtittle_row}`}>
				...
				<button className={styles.delete_button} onClick={()=>handleRemoveDice()}>Remove dice</button>
			</div>
			<DiceTray pool={pool} />
			...
			<div className={`${styles.roll_row}`}>
				<button className={styles.main_button} onClick={()=>pool.rollAll()}>
					Roll Dice
				</button>
